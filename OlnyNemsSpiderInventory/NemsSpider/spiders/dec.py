...
from NemsSpider.items import DecspiderItem
...
class DecSpider(scrapy.Spider):
    name = 'dec'
    allowed_domains = ['sz.singlewindow.cn']
    start_urls = r'http://sz.singlewindow.cn/dyck/swProxy/decserver/sw/dec/merge/cusQuery?{}'
    edit_url = r"http://sz.singlewindow.cn/dyck/swProxy/decserver/sw/dec/merge/queryDecData"  # POST

    def __init__(self, input_date=None, is_today='True', *args, **kwargs):
        super(DecSpider, self).__init__(*args, **kwargs)
            ...
            for one_time in self.input_date:
                ...
                for one_ie in ie_data:
                    ...
                    for one_start in start_data:
                        ...
                        yield postdata3
        cookies = json.loads(pyredis.get("dec_request_cookies"))
        postdata = {
            'cusCiqNoHidden': '',
            'dclTrnRelFlagHidden': '',
            'transPreNoHidden': '',
    ...
    def parse(self, response):
        ...
        if data.get("total") != now_count:
            log.warning(f"{first_post_data.get('updateTime')}-{first_post_data.get('updateTimeEnd')}:应该获取的总数：{data.get('total')}条 实际获取的总数：{now_count}条")
    ...
    def parse_item(self, response):
        ...
        try:
            ...
            log.error(f'url: {response.request.url} de 返回值有问题：{response.text}， {traceback.format_exc()}')
            raise e
